...
def create_ui():
    if cmds.window("followControlUI", exists=True):
        cmds.deleteUI("followControlUI")

    cmds.window("followControlUI", title="Follow Control UI", widthHeight=(300, 150))

    cmds.columnLayout(adjustableColumn=True)
    cmds.text(label="Select Parent Control and Child Control then click 'Create Follow Controls'")
    cmds.separator(height=10, style='none')
    cmds.button(label="Create Follow Controls", command=create_follow_controls)

    cmds.showWindow("followControlUI")


def create_follow_controls(*args):
    # get selection, separate parent control and child control
    sels = cmds.ls(sl=True)  # [parent control, child control]
    if len(sels) != 2:
        cmds.warning("Please select a parent control and a child control.")
        return
